...
export function buildStatementHtml(params: {
  ...
  const carryRow = carriedOver > 0
    ? `<tr><td style="padding: 8px 0; color: #444;">前月繰越</td><td style="padding: 8px 0; text-align: right;">¥${carriedOver.toLocaleString()}</td></tr>`
    : "";
...
<div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 24px; color: #333;">
  ...
  <table style="width: 100%; font-size: 13px; margin-bottom: 24px;">
    <tr>
      <td style="color: #444;">発行日</td>
      <td style="text-align: right; color: #222;">${escapeHtml(issuedDate)}</td>
    </tr>
    <tr>
      <td style="color: #444;">明細書番号</td>
      <td style="text-align: right; color: #222; font-family: monospace;">${escapeHtml(statementNumber)}</td>
    </tr>
  </table>
  ...
  <p style="font-size: 13px; color: #444; margin-bottom: 32px;">お支払方法: 銀行振込</p>

  <div style="border-top: 1px solid #ddd; padding-top: 20px; font-size: 12px; color: #444;">
    <p style="margin: 0 0 4px;">発行者:</p>
    <p style="margin: 0 0 2px;">SATOYAMA AI BASE / 星の図書館</p>
    <p style="margin: 0 0 2px;">稲福良祐</p>
